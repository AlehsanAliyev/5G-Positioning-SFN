...
DL_FEATURES = [
    ...
    "NR_UE_Nbr_PCI_1", "NR_UE_Timing_Advance",
    "NR_UE_Nbr_RSRP_1",
]
...
PCI_BASES = ["NR_UE_PCI_0", "NR_UE_Nbr_PCI_0", "NR_UE_Nbr_PCI_1"]
PCI_GEOM_BASES = ["NR_UE_PCI_0", "NR_UE_Nbr_PCI_0"]
EARTH_RADIUS_M = 6_371_000
MISSINGNESS_CUTOFF = 0.9
EPS = 1e-6
RSRP_RANGE = (-140, -60)
RSRQ_RANGE = (-30, -5)
SINR_RANGE = (-10, 40)
...
def is_useless_signal(row):
    ...
    existing = [c for c in critical_cols if c in row.index]
    if existing and row[existing].isna().all():
        return True
...
def mask_missing_geometry(df, pci_col_base):
    flag_col = f"{pci_col_base}_bs_found"
    if flag_col not in df.columns:
        return df
    mask = df[flag_col] == False
    geom_cols = [
        f"{pci_col_base}_lat",
        f"{pci_col_base}_lon",
        f"{pci_col_base}_azimuth",
        f"{pci_col_base}_height",
        f"{pci_col_base}_distance",
        f"{pci_col_base}_bearing",
        f"{pci_col_base}_angle_offset",
        f"{pci_col_base}_cos_offset",
        f"{pci_col_base}_rsrp_weighted",
    ]
...
def report_pci_match_rate(df):
    pci_cols = [c for c in df.columns if c.endswith("_bs_found")]
    if not pci_cols:
        print("No PCI match-rate columns found.")
        return
    for col in pci_cols:
        rate = df[col].mean() * 100
        print(f"{col}: {rate:.2f}% matched")


def drop_high_missing(df, cutoff):
    missing = df.isna().mean()
    drop_cols = missing[missing > cutoff].index.tolist()
    if drop_cols:
        df = df.drop(columns=drop_cols)
    return df


def clip_signal_ranges(df):
    for col in df.columns:
        if "RSRP" in col:
            df[col] = df[col].clip(RSRP_RANGE[0], RSRP_RANGE[1])
        elif "RSRQ" in col:
            df[col] = df[col].clip(RSRQ_RANGE[0], RSRQ_RANGE[1])
        elif "SINR" in col:
            df[col] = df[col].clip(SINR_RANGE[0], SINR_RANGE[1])
    return df


def add_delta_features(df):
    if "NR_UE_RSRP_0" in df.columns and "NR_UE_Nbr_RSRP_0" in df.columns:
        df["RSRP_delta_0_0"] = df["NR_UE_RSRP_0"] - df["NR_UE_Nbr_RSRP_0"]
    if "NR_UE_RSRQ_0" in df.columns and "NR_UE_Nbr_RSRQ_0" in df.columns:
        df["RSRQ_delta_0_0"] = df["NR_UE_RSRQ_0"] - df["NR_UE_Nbr_RSRQ_0"]
    if "NR_UE_SINR_0" in df.columns and "NR_UE_Nbr_SINR_0" in df.columns:
        df["SINR_delta_0_0"] = df["NR_UE_SINR_0"] - df["NR_UE_Nbr_SINR_0"]
    return df


def add_geometry_interactions(df, pci_col_base):
    dist_col = f"{pci_col_base}_distance"
    cos_col = f"{pci_col_base}_cos_offset"
    rsrp_w_col = f"{pci_col_base}_rsrp_weighted"
    if dist_col in df.columns and cos_col in df.columns:
        df[f"{pci_col_base}_dist_cos"] = df[dist_col] * df[cos_col]
    if rsrp_w_col in df.columns and dist_col in df.columns:
        df[f"{pci_col_base}_rsrp_over_dist"] = df[rsrp_w_col] / (df[dist_col] + EPS)
    return df
...
def load_downlink_series_data():
    ...
    report_pci_match_rate(df)

    for pci_base in PCI_GEOM_BASES:
        if f"{pci_base}_lat" in df.columns:
            df = apply_geometry_features(df, pci_base)
            df = mask_missing_geometry(df, pci_base)
            df = add_geometry_interactions(df, pci_base)

    df = clip_signal_ranges(df)
    df = add_delta_features(df)
    df = drop_high_missing(df, MISSINGNESS_CUTOFF)
